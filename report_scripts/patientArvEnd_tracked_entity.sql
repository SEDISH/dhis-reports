...
DELIMITER $$
CREATE PROCEDURE patientArvEnd_tracked_entity(IN org_unit VARCHAR(11))
BEGIN
...
FROM (SELECT CONCAT('[', instance.array, ']') as entity_instance
  FROM (SELECT GROUP_CONCAT(entities_list.track_entity SEPARATOR ',') AS array
    FROM (
      ...
      FROM
      isanteplus.patient p, isanteplus.patient_dispensing pdisp, isanteplus.tmp_idgen tmp,
      (SELECT pad.patient_id, MAX(pad.next_dispensation_date) as next_dispensation_date
        FROM isanteplus.patient_dispensing pad GROUP BY 1) B
